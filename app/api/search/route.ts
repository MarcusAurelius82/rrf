...
import { searchWithAI, extractStateFromQuery } from "@/lib/anthropic";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. State extracted from the query by Haiku ("sf"→CA, "nyc"→NY, "Chicago"→IL)
    // 3. Fall back to US-wide
    const resolvedState: string | null =
      state ?? (await extractStateFromQuery(query));
